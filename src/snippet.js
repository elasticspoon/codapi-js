...
import boxes from "./engine/wasi/boxes.js";
...
class CodapiSnippet extends HTMLElement {
  ...
  // Returns available versions for current sandbox
  get versions() {
    if (!this.sandbox) return [];
    const [sandboxName] = text.cut(this.sandbox, ":");
    const box = boxes[sandboxName];
    if (!box?.versions) return [];
    return Object.keys(box.versions);
  }

  // Changes to specific version
  setVersion(version) {
    if (!this.sandbox) return;
    const [sandboxName] = text.cut(this.sandbox, ":");
    this.sandbox = `${sandboxName}:${version}`;
  }
